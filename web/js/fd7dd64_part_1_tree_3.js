...
function setupTree(apiPath, sitePath) {
  ...
  // One tree to display the ancestors /*******************************************************************ANCESTORS inversion 1 / -1 => symétrie gauche droite
  ...
  // Use a separate tree to display the descendants /****************************************************DESCENDANTS
...
Tree.prototype.drawLinks = function(links, source){
  ...
  link.enter().append("path")
      ...
      .attr("d", function(d) {
        var o = {/**x**/y: source.x0, /**y**/x: self.direction * (source.y0 + boxWidth/2)};
...
Tree.prototype.drawNodes = function(nodes, source, sitePath){
  ...
  //par là pour changer orientation-------------------------------------------------------------------------------
  var nodeUpdate = node.transition()
      .duration(duration)
      //.attr("transform", function(d) { return "translate(" + (self.direction * d.y) + "," + d.x + ")"; });
      .attr("transform", function(d) { return "translate(" + (self.direction * d.x) + "," + (d.y * (-1 /*symétrie horizontale*/)) + ")"; });
  ...
  var nodeExit = node.exit()
      ...
      .attr("transform", function(d) { return "translate(" + (self.direction * (source.x + boxWidth/2)) + "," + (source.y * (-1))+ ")"; })
...
function elbow(d, direction) {
   var sourceY = d.source.x ,
      sourceX = (d.source.y + (boxWidth / 2))* (-1),
      targetY = d.target.x ,
      targetX = (d.target.y - (boxWidth / 2))* (-1);
